...
public class OrderStatus
    : Enumeration
{
    public static readonly OrderStatus Initial = new OrderStatus(0, nameof(Initial).ToLowerInvariant());
    ...
    public static OrderStatus FromName(string name)
    {
        ...
        if (state == null)
        {
            throw new OrderMgmtException($"Possible values for OrderStatus: {string.Join(",", List().Select(s => s.Name))}");
        }
